...
///////////////////////////////////////////////////////////////////////////////
    ...
    type CPU () =
        ...
        static member private PodzielNaKolory (reka: Karta list) =
            [Kier; Karo; Trefl; Pik]
                |> List.map (fun k -> (List.filter (fun (Karta(ko, _)) -> ko = k) reka, k)) 
        
        static member private InformacjeOKolorze (reka: Karta list, kolor: Kolor) = 
        ...
        static member private OszacujPunkty (reka: Karta list) =
            ...
            let infoOKolorach = CPU.PodzielNaKolory reka |> List.map CPU.InformacjeOKolorze
        ...
        static member private SprawdzMeldunki (reka: Karta list) = 
            CPU.PodzielNaKolory reka
                |> List.filter (fun (karty, kolor) -> znajdzMeldunek kolor karty)
                |> List.map snd

        override this.Licytuj (reka: Karta list) (wynik: Int32) = 
            let przewidywana = ((CPU.OszacujPunkty reka) / 10) * 10
        ...
        static member private WygrywajaceKarty (reka: Karta list) (stol: Stol) =
        ...
        override this.ZagrajKarte (reka: Karta list) (stol : Stol) (gracze: Gracz []) (kolejnosc: Int32 list) =
            ...
            let meldunki = CPU.SprawdzMeldunki dozwolone
            let mozeMeldowac = List.isEmpty stol.karty && not <| List.isEmpty meldunki
            match CPU.WygrywajaceKarty dozwolone stol with
            | (x::_, _)                 -> x
